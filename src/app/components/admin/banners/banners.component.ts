...
export class BannersComponent implements OnInit {
  ...
  constructor(private bannersAPI: BannersService, private notification: NotificationService, private fileUploadAPI: FileUploadService, protected configService: AppConfigService, private fb: FormBuilder) {
    this.bannersForm = this.fb.group({
      id: [null],
      type: [null],
      name: [null],
    });
    this.breadcrumbItem = [{ label: 'Quản lý banners' }, { label: 'Danh sách banners' }];

    this.home = {
      icon: 'pi pi-home',
      routerLink: '/admin/admin-dashboard',
    };
    this.basefileUrl = this.configService.getConfig().api.fileUrl;
    console.log(this.basefileUrl);

  }

  ngOnInit(): void {
    this.cols = [{ field: 'name', header: 'Tên banner', width: '16rem' }];
    this.search();
  }
